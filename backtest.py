...
class BackTester():
    ...
    def mainpart(self):
        ...
        data = pd.read_csv("30days_ohlcv.csv",
                           index_col='datetime',
        ...
        for i, v in data.iterrows():
            ...
            if current['pos'] == 'none':
                if self.threshold['div'] > abs(current['div_long']):
                ...
                if self.order['profitline'] < current['price']:   #利確
                    ...
                    money = money + (self.order['size'] * self.order['profitline'])
                    ...
                    money = money + (self.order['size'] * self.order['stopline'])
                    series = pd.Series([money, current['time']], index=account.columns)
                    account = account.append(series, ignore_index=True)
